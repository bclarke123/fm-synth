...
fn run<T: Sample>(device: Device, config: StreamConfig) -> Stream {
    ...
    let data_callback = |data: &mut [T], _info: &cpal::OutputCallbackInfo| {
        ...
        if let (Some(carrier), Some(modulator)) = (c_lock.as_deref_mut(), m_lock.as_deref_mut()) {
            for sample in data.iter_mut() {
                let frame = (carrier.next() + modulator.next()) as f32;
